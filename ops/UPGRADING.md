...
- The `PackageReference` lives in `Mailvec.Parsing` (the one project that holds every mail-content parser; it absorbed the former `Mailvec.Pdf`), reached only through `IMailParser` by `Mailvec.Mcp` (`get_attachment_page_image`, `view_attachment` image normalisation) and `Mailvec.Embedder` (the scanned-PDF / image OCR pass). `Mailvec.Core` must stay free of every parser package — `ParserBoundaryTests` enforces it. The MCP test project also references `SkiaSharp` directly to decode rendered images.
