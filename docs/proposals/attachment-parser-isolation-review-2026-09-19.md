...
> `ParseEndpoints`' catch-all comment. Per-finding notes below. Test state after: 1,401 passing
> (the 1,404 first written here was a guess and is the fifth pass's A5).
...
# Addendum — fifth pass, reviewing the fixes (`dbb8fc8`)

Reviewer: the fourth-pass reviewer, re-checking the fix commit. `dotnet build` clean; **1,401
passing, 0 failing** across six projects (the status doc says 1,404 — see A5). F1 was
independently re-reproduced against the new code with a clean loopback harness (proper project,
not a file-based app — see the caveat below), at the original failing sizes.

## Disposition of the fourth-pass findings

| Finding | Result |
| --- | --- |
| F1 — over-cap message misclassified `Crashed` | **Closed, verified at the original sizes.** 8 MB and 40 MB against a 4 MB host cap: the drifted mirror gets `DocumentRejected` via the 413 answered at the headers (the 40 MB body never crosses — 0.7 ms); the aligned mirror refuses pre-send in 0 ms with no network call. Under-cap requests flow normally through `Expect: 100-continue`. Both sides bind the same `Parser:MaxRequestBodyBytes` key, so one setting feeds both and env-level drift is impossible; the `ExpectContinue` backstop covers a default change on one side only. |
| F2 — client disconnect recycles the host | **Closed.** The wait is untied from `RequestAborted`; the gate slot is released in the parse task's `finally`, so it survives the request being abandoned; only a genuine overrun exits. Mutation-verified test (`A_caller_that_disconnects_mid_parse_does_not_take_the_host_down`). |
| F3 — no admission control | **Closed.** `ParseGate` (`Parser:MaxConcurrentParses`, 4); slot-full within the timeout is 503 `busy` → `Unavailable` to the caller, never a strike. Real-Kestrel tests for both serialization and the 503 classification. |
| F4 — poison MIME structure | **Closed.** The give-up path calls no parser and writes no marker, keeps the file out of deletion reconciliation, and counts it honestly as `Failed`. The `2 × MaxCrashesPerFile` threshold matches the "half on the metadata-only parse" comment. |
| F5 — per-part re-upload | Deferred to status-doc open question 1 (measure first). Reasonable. |
| F6 — drift | Fixed, with one new instance (A5). |
| Accepted asymmetry | Now stated in `ParseEndpoints`' catch-all comment. |

**Harness caveat, for the record:** an intermediate re-probe reported a spurious host-side 500 for
*every* request. That was the probe, not the fix — `dotnet run file.cs` file-based apps default to
source-generated JSON (`TypeInfoResolver of type '[]'`), so the probe host's route table never
built and Kestrel 500'd before any body-cap logic ran. Redone as a normal project the same probe
produces the table above. (It also means the fourth pass's original `Crashed` reproduction saw a
mid-upload send failure triggered by an early 500 rather than an early 413 — the mechanism the
finding described, an unanswered upload racing an early response, is the same, and the in-repo
test `A_message_over_the_host_cap_but_under_the_mirror_is_still_DocumentRejected` exercises the
real 413.)

## New findings — all minor, all in the fix commit

### A1. [P3] The client-timeout invariant is stale, and slightly false

**Location:** `src/Mailvec.Core/Options/ParserOptions.cs` (`RequestTimeoutSeconds`, 90 s) vs.
`ParseEndpoints.Execute` (gate wait ≤ `RequestTimeoutSeconds`, then parse ≤ `RequestTimeoutSeconds`).

The comment still says the host always answers within its own 60 s, so a client timeout "means the
host never answered at all" (→ `Unavailable`). With the gate, host-side handling can now be
gate-wait (≤60 s) **plus** parse (≤60 s) = 120 s > 90 s, so a client timeout can fire while the
host is legitimately queued or mid-parse. Benign — it classifies `Unavailable`, never a strike,
and the host finishes and discards the result — but the invariant the comment states is what a
future tuning decision will lean on.

**Suggested fix:** bump the client default above gate + parse (e.g. 150 s), or reword the comment.

### A2. [P3] `ParseGate` slot leak if `Task.Run` itself throws

**Location:** `src/Mailvec.Parse/ParseEndpoints.cs` (`Execute`): admission happens, then
`Task.Run(() => { try { … } finally { gate.Exit(); } })`.

If task creation itself throws after admission (thread-injection failure at `pids_limit: 128`),
`gate.Exit()` never runs and the slot leaks. Four leaks and the host 503s every parse forever —
and because 503s never count toward `RequestBudget`, the host never exits, `/up` stays green and
the compose healthcheck passes while every caller sees a permanent `Unavailable`. Very rare, but
the failure mode is invisible to exactly the monitoring that would be asked about it.

**Suggested fix:** a `try/catch` around the `Task.Run` that releases the slot on throw.

### A3. [P3] The 499 path logs "completed" even when the parse faulted, and swallows the exception

**Location:** `src/Mailvec.Parse/ParseEndpoints.cs` (`Execute`, the
`ctx.RequestAborted.IsCancellationRequested` branch after a finished parse).

The log line says "completed after its caller disconnected; result discarded", then
`try { await task; } catch (Exception) { }` discards a fault silently. A failed-after-disconnect
parse is now the one document failure the host never logs anywhere — the timeout and 422 paths
both log.

**Suggested fix:** log the exception at warning even when the caller is gone.

### A4. [P3] `/v1/html` has neither the pre-flight nor `Expect: 100-continue`

**Location:** `src/Mailvec.Core/Parsing/RemoteParser.cs` (`BodyTextFromHtml` builds its own
request, unlike `PostEml`).

A stored `body_html` over the cap would hit the original mid-upload race and classify `Crashed`.
It needs a >48 MB HTML part, which a container-indexed archive cannot have — only a macOS-seeded
one — so theoretical, but one `request.Headers.ExpectContinue = true` makes it consistent.

### A5. [P3] Test-count drift, again

The suite totals **1,401**; the status doc header says 1,404. (F6 was about exactly this.)

---

*Method note: read-only review of `1e7fa34..dbb8fc8` plus a local loopback re-reproduction of F1
...
> A5: the status header now carries the counted total, 1,402 after this round.
