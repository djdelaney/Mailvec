...
Search can also **filter by attachments**: `search_emails` takes `hasAttachments` (true/false) and `attachmentType` (the token `image` — any `image/*` MIME or common image filename extension — or a filename extension like `pdf` / `docx`, matched against the filename suffix *or* the extension's known MIME, so mislabeled `application/octet-stream` attachments still match). Both work with or without a query, so "what PDFs did I get last month" is a plain filtered browse.
