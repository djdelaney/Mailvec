...
- The MCP server reads `~/Library/Application Support/Mailvec/archive.sqlite` on every tool call. macOS sees "Claude-identified helper reading inside `~/Library/Application Support/<a different app>/`" → `kTCCServiceSystemPolicyAppData`.
- TCC caches the decision per-binary, so the prompt can fire long after install rather than at a moment tied to a user action. (Historically a 60 s background sampler kept re-triggering it on an idle machine; that sampler was tray-only and is gone, so an idle server now makes no DB reads at all.)
