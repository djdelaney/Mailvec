...
## `embeddingProvider.ready` — provider-neutral readiness

The `/up` response includes:

```json
{ "embeddingProvider": { "ready": true }, "ollama": { "reachable": true } }
```

In versions that include the field, both values carry the configured embedding
profile's real-embed probe result; `ollama.reachable` is a compatibility alias.
Before changing a monitor, fetch `/up` through the deployed tunnel and confirm `embeddingProvider.ready`
is present, then inspect the monitor's current JSON query in Kuma. Repoint any
monitor still using `ollama.reachable` and verify its next poll. The alias
remains until a separate wire-contract change after monitor migration.
