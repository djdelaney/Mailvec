...
- **Extraction libraries are pure-managed.** PDF via `PdfPig` (Apache 2.0); DOCX via `DocumentFormat.OpenXml` (MIT). No native deps, no shell-out, no OCR. Scanned PDFs come back as `extraction_status='no_text'` and are intentionally not retried. **This applies to the indexer's text-extraction path only.** The MCP `get_attachment_page_image` tool rasterises a PDF page to JPEG via `PDFtoImage` (PDFium + SkiaSharp, **native**) — the only native dep in the read path besides `sqlite-vec`, referenced **solely** by `Mailvec.Mcp` (not Core/Indexer/Embedder/Cli) and platform-gated with `[SupportedOSPlatform]` to macOS/Linux/Windows. Native assets for `osx-arm64`/`osx-x64`/`linux-x64`/`linux-arm64` come transitively through NuGet (no fetch script, unlike `vec0.dylib`); they bloat the self-contained MCPB bundle. The renderer (`PdfRenderer`) caps the long edge at 1536px (downscale-only) and encodes JPEG q85 on a white background — a fixed-DPI PNG let a large-MediaBox scan balloon to ~20MB, pointless since Claude downsamples to ~1568px anyway. Rendering is also how a `no_text` (scanned/image-only) PDF becomes readable — handed to Claude's vision instead of an OCR engine.
