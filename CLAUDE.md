...
- **`part_index` maps to a MIME part via `MessageParts.Indexable(mime)` — the single source of truth shared by the writer (`MessageParser`) and the reader (`MaildirAttachmentReader`, hence MCP `get_attachment` + the OCR pass).** The set is `mime.Attachments` (disposition=attachment) **first**, then inline image parts (MimeKit's `Attachments` excludes `Content-Disposition: inline` / `cid:` images — commonly real content: photographed docs pasted inline by Apple Mail, forwarded post images). **Inline images are appended after the attachments, not interleaved**, so existing rows' `part_index` never shifts — capturing them is an additive backfill (`mailvec backfill-inline-images`), not a reindex. If the two enumerations ever diverge, `get_attachment`/OCR silently read the wrong bytes for a part. Inline images extract to `unsupported`, so the embedder's image-OCR pass picks them up under its gate. The backfill's candidate set is `body_html LIKE '%cid:%'` (bounded; the inline-ref pattern) — it misses inline images in HTML-less mail, a rare edge.
