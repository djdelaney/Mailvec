...
- **v3 → v4 upgrade path: drop the DB.** `004_attachment_text.sql` adds the columns but cannot backfill `extracted_text` for existing attachments — that requires re-walking each `.eml` file, which the mtime fast-path will skip on rescan since mtime hasn't changed. The supported path is `rm archive.sqlite` and let the indexer rebuild. (A `mailvec extract-attachments` CLI for in-place backfill would be a clean follow-up but isn't in scope for v4.)
