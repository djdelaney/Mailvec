...
Phases 1, 2, and 3 (in progress) complete. The indexer ingests, the embedder embeds, keyword/semantic/hybrid search all work, and the MCP server exposes `search_emails` over both Streamable HTTP (`127.0.0.1:3333`, default) and stdio (`--stdio` flag, used by Claude Desktop). Claude Desktop is wired up via `~/Library/Application Support/Claude/claude_desktop_config.json` → `ops/run-mcp-stdio.sh`. Remaining Phase 3 tools (`get_email`, `get_thread`, `list_folders`, `find_by_sender`, `recent_emails`) are not yet implemented.
...
- **Search filters live on the search services in Core, not on the MCP tool.** `KeywordSearchService.Search`, `VectorSearchService.SearchAsync`, and `HybridSearchService.SearchAsync` all accept a `SearchFilters?` (`Folder` / `DateFrom` / `DateTo` / `FromContains`). The shared `SearchFilterSql.Append` helper appends WHERE clauses and binds the params so filter semantics stay identical across the BM25 and vector legs — important because hybrid RRF fuses two ranked lists, and post-filtering only one side would skew scores. **Don't** post-filter results in the MCP layer.
