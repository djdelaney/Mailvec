...
- **Response field names** that clients narrate to users: `matchedAttachment.{partIndex,fileName}`, `archiveStats.{totalMessages,oldestDate,latestDate}`, `appliedFilters.*`, `get_thread`'s `{count,totalCount,truncated}` + per-entry `bodyTruncated` (**`count` means "entries in `messages`" and always equals its length** — the full thread size is `totalCount`; don't "fix" `count` to mean the thread size, that silently contradicts the array clients iterate), `get_email`'s `bodyTruncated` / `bodyTextChars` (added 2026-09-23 — an additive response change, which still counts as a tool-surface change for the version bump), `webmailUrl`, `webmailLink` (a pre-escaped Markdown link, built by `WebmailLinkBuilder.MarkdownLink` and rendered verbatim by clients — the tools construct it server-side precisely so the untrusted subject can't be assembled into a spoofed link by the model; the tool descriptions tell clients to render it, not to build their own).
- **Server identity**: `serverInfo.name = "mailvec"` (lowercase, the protocol identifier — client configs and the connector registration key off it). Bump `serverInfo.version` whenever you ship a tool-surface change so a client log line of "I'm talking to mailvec 0.1.16" tells you which build you're seeing.
...
- **MCP runs in two transport modes** sharing the same Core wiring (`AddMailvecServices` helper in `Program.cs`):
  ...
  - **Stdio (`--stdio` flag)**: Generic `Host` + `WithStdioServerTransport()`. Used by Claude Desktop because its connector schema only supports stdio. **Wire a real client up with `ops/install-stdio-launcher.sh`**, which publishes Release and writes a stable shim at `~/.local/bin/mailvec-mcp-stdio` that inherits the shared `appsettings.Local.json`. `ops/run-mcp-stdio.sh` is the *dev-iteration* launcher — Debug build from the working tree — not the one to put in a client config.
