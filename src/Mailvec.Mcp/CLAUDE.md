...
Tool names, parameter names, and response field names are a **contract** — renames break every client at once. This used to read "once Phase 5 starts (Gemini CLI / Codex CLI / ChatGPT desktop pointing at the same server)", i.e. as a lock that hadn't engaged yet. **It has, and dropping Phase 5 did not loosen it — the opposite.** One OAuth-gated connector now serves Claude Code, Desktop, iOS and claude.ai simultaneously, plus any MCPB/stdio install, so there is no longer a staging window in which a rename breaks one client and you notice before it reaches the rest. Treat this list as locked unless you're deliberately bumping the version:
